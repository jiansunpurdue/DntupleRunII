...
double rebin[REBINp] = {60.,100.};
//double rebin[REBINp] = {50,55,60,65,70,80,100,150};


void DzerodsigmadptPbPbBinning(TString input="pp_d0meson5_5TeV_y1",double norm=0.557)
{
  norm=norm*208.*208.;
  ...
  TFile*foutput=new TFile(Form("../Results/output_%s_PbPbBinning.root",input.Data()),"recreate");
  foutput->cd();
  gaeSigmaDzero->Write();
