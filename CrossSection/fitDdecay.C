...
void fitDdecay(TString inputdata="/data/wangj/MC2015/Dntuple/backup/ntD_EvtBase_20160125_Dfinder_20151229_pp_Pythia8_prompt_D0_pthatweight.root", TString inputmc="/data/wangj/MC2015/Dntuple/backup/ntD_EvtBase_20160125_Dfinder_20151229_pp_Pythia8_prompt_D0_pthatweight.root", TString trgselection="1",  TString cut="Dy>-1.&&Dy<1.&&(Dtrk1highPurity&&Dtrk2highPurity)&&Dchi2cl>0.05&&Dalpha<0.12&&Dtrk1Pt>1.5&&Dtrk2Pt>1.5", TString selmcgen="((GisSignal==1||GisSignal==2)&&(Gy>-1&&Gy<1))", int isMC=0, Double_t luminosity=26., int doweight=0, TString collsyst="PbPb", TString outputfile="mytest.root")
{
  ...
  ntMC->Project("hPtMC","(DsvpvDistance/DsvpvDisErr)",TCut(weight)*(TCut(selmc.Data())&&"(Dgen==23333)"));
  divideBinWidth(hPtMC);
  ntMC->Project("hPtRecoTruth","(DsvpvDistance/DsvpvDisErr)",TCut(selmc.Data())&&"(Dgen==23333)");
...
TF1* fit(TTree* nt, TTree* ntMC, Double_t ptmin, Double_t ptmax, int isMC)
{
  ...
  if(isMC==1) nt->Project(Form("h-%d",count),"Dmass",Form("%s*(%s&&(DsvpvDistance/DsvpvDisErr)>%f&&(DsvpvDistance/DsvpvDisErr)<%f)",weight.Data(),seldata.Data(),ptmin,ptmax));   
  else nt->Project(Form("h-%d",count),"Dmass",Form("(%s&&(DsvpvDistance/DsvpvDisErr)>%f&&(DsvpvDistance/DsvpvDisErr)<%f)",seldata.Data(),ptmin,ptmax));   
  
  ntMC->Project(Form("hMCSignal-%d",count),"Dmass",Form("%s*(%s&&(DsvpvDistance/DsvpvDisErr)>%f&&(DsvpvDistance/DsvpvDisErr)<%f&&(Dgen==23333))",weight.Data(),selmc.Data(),ptmin,ptmax));   
  ntMC->Project(Form("hMCSwapped-%d",count),"Dmass",Form("%s*(%s&&(DsvpvDistance/DsvpvDisErr)>%f&&(DsvpvDistance/DsvpvDisErr)<%f&&(Dgen==23344))",weight.Data(),selmc.Data(),ptmin,ptmax));   
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 11)
    fitDdecay(argv[1], argv[2], argv[3], argv[4], argv[5], atoi(argv[6]), atof(argv[7]), atoi(argv[8]),argv[9],argv[10]);
  return 0;
}
